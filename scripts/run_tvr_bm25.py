...
from src.retrieval import build_bm25_index, retrieve_top_k_windows
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run a subtitle-window BM25 baseline on TVR.")
    ...
    parser.add_argument("--output-dir", default="outputs/tvr/bm25")
    ...
    for query in tqdm(queries, desc="TVR BM25"):
        vid_name = str(query["vid_name"])
        windows = windows_by_video.get(vid_name, [])
        if vid_name not in bm25_cache and windows:
            bm25_cache[vid_name] = (windows, build_bm25_index(windows)[0])

        with Timer() as timer:
            if windows:
                cached_windows, bm25 = bm25_cache[vid_name]
                retrieved_windows = retrieve_top_k_windows(
        ...
        predictions.append(
            {
                ...
                "retrieval_time_ms": timer.elapsed_s * 1000.0,
            }
        )
    ...
    summary.update(
        {
            "split": args.split,
            "baseline": "bm25_subtitle_windows",
